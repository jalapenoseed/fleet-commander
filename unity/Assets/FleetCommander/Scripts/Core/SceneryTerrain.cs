...
namespace FleetCommander.Core
{
    ...
    public static class SceneryTerrain
    {
        ...
        {
            if(c.planet!=PlanetKind.Earth||c.scenery==SceneryKind.Stadium)return 0;
            ...
            if(c.scenery==SceneryKind.City){float townEdge=Mathf.Max(Mathf.Abs(x)-430,Mathf.Abs(z-225)-65);h*=Mathf.SmoothStep(0,1,Mathf.Clamp01(townEdge/80));}
            if(c.scenery==SceneryKind.Coast){float land=Mathf.SmoothStep(0,1,Mathf.Clamp01((x+210)/100));h=Mathf.Lerp(-.7f,h,land);}
            if(c.scenery==SceneryKind.River){float center=270+Mathf.Sin(z*.009f)*50,bank=Mathf.SmoothStep(0,1,Mathf.Clamp01((Mathf.Abs(x-center)-25)/28));h=Mathf.Lerp(-.35f,h,bank);}
            return h;
        }
    }
}
