...
namespace FleetCommander.Games
{
    ...
    [Serializable] public sealed class MatchResults
    {
        ...
        public void Load(){try{if(File.Exists(PathName)){var saved=JsonUtility.FromJson<MatchResults>(File.ReadAllText(PathName));matches=saved?.matches??new List<MatchResult>();matches.RemoveAll(m=>m==null);if(matches.Count>200)matches=matches.GetRange(matches.Count-200,200);foreach(var m in matches){if(m.mode=="Soccer")m.mode="SOCCER";else if(m.mode=="CaptureTheFlag")m.mode="CAPTURE THE FLAG";else if(m.mode=="FlagFootball")m.mode="FLAG FOOTBALL";}}}catch(Exception e){Debug.LogWarning("Results could not be loaded: "+e.Message);}}
