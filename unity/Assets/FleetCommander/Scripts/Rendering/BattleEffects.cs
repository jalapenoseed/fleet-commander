...
namespace FleetCommander.Rendering
{
    public sealed class BattleEffects : MonoBehaviour
    {
        ...
        void Update(){if(Simulator.Replay.Playing){effects.Clear();return;}if(Simulator.Paused)return;for(int i=effects.Count-1;i>=0;i--){var e=effects[i];e.life-=Time.deltaTime;if(e.life<=0)effects.RemoveAt(i);else effects[i]=e;}}
        void OnRenderObject()
        {
