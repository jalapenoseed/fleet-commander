...
function resolve(s) {
  s = s.replace(/#include <(\w+)>/g, (_, k) => resolve(T.ShaderChunk[k] || ''));
  for (const [k, v] of Object.entries(counts).sort((a, b) => b[0].length - a[0].length))
    s = s.replaceAll(k, String(v));
  s = s.replace(
    /#pragma unroll_loop_start\s+for\s*\(\s*int i = (\d+); i < (\d+); i\s*\+\+\s*\)\s*{([\s\S]+?)}\s*#pragma unroll_loop_end/g,
    (_, a, b, body) =>
      Array.from({ length: Number(b) - Number(a) }, (_, j) =>
        body
          .replace(/\[\s*i\s*\]/g, '[ ' + (j + Number(a)) + ' ]')
          .replace(/UNROLLED_LOOP_INDEX/g, String(j + Number(a))),
      ).join(''),
  );
  return s;
}
function pack(name, vs, frag, physical = false) {
...
for (const kind of ['concrete', 'grass', 'glass', 'water', 'stone', 'metal']) {
  const m = surfaceMaterial('#aaa', kind),
    s = {
      uniforms: {},
  ...
  m.onBeforeCompile(s);
  pack(kind, s.vertexShader, s.fragmentShader, true);
}
const scene = new T.Scene(),
  env = new DirectorEnvironment(scene, {});
pack('sky', env.dome.material.vertexShader, env.dome.material.fragmentShader);
env.dispose();
const post = new ScenePostFX({ extensions: { has: () => true }, capabilities: { maxSamples: 4 } });
for (const key of ['blur', 'composite'])
  pack(key, post[key].vertexShader, post[key].fragmentShader);
post.dispose();
const root = new T.Scene(),
  beacons = new AircraftBeacons(root),
  lighting = new SwarmLighting(root);
for (const [key, m] of [
  ['beacons', beacons.points.material],
  ['ground-pools', lighting.pools.material],
])
  pack(key, m.vertexShader, m.fragmentShader);
beacons.dispose();
lighting.dispose();
const effects = new BattleEffects(root);
pack(
  'battle-particles',
  effects.points.material.vertexShader,
  effects.points.material.fragmentShader,
);
effects.dispose();
fs.mkdirSync('qa-output', { recursive: true });
fs.writeFileSync('qa-output/shaders.json', JSON.stringify(shaders));
console.log(
  'Exported',
  shaders.length,
  'shader programs, including physical lighting, fog, instancing and shadows.',
);
