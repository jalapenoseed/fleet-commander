...
import {MAX_COMMANDER_DRONES} from './fleet-commander-core.js?v=0.8.0';
...
export class AircraftBeacons{
 constructor(root){
  ...
  const material=new T.ShaderMaterial({uniforms:{size:{value:9},time:{value:0},steady:{value:0},intensity:{value:1},haze:{value:1}},vertexShader:`
   ...
   }`,transparent:true,depthWrite:false,depthTest:true,blending:T.AdditiveBlending,toneMapped:true});
 ...
 update(drones,camera,{time=0,size=9,pixelRatio=1,reducedMotion=false,intensity=1,haze=1}={}){
  const a=this.points.geometry.attributes;
  for(let i=0;i<drones.length;i++){
   const d=drones[i],pos=d.pos,active=!['DOCK','QUEUED','LANDED'].includes(d.mode);a.position.setXYZ(i,pos[0],pos[1]+.22,pos[2]);
   const key=d.beaconHex||d.color;if(this.colors.size>8192)this.colors.clear();if(!this.colors.has(key))this.colors.set(key,new T.Color(d.beaconHex||beaconColor(d.color).hex));const color=this.colors.get(key);
  ...
  this.points.material.uniforms.intensity.value=intensity;this.points.material.uniforms.haze.value=haze;
 }
