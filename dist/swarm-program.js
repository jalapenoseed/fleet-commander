import {showPosition,DIRECTOR_STYLES} from './show-motion.js?v=0.8.0';
import {BOID_DEFAULTS,BOID_RANGES} from './boids.js?v=0.8.0';
import {DEFAULT_FLEET_SONG,parseFleetSong} from './fleet-score.js?v=0.8.0';
import {STARTER_AIRCRAFT} from './fleet-manifest.js?v=0.8.0';
import {parseFormula,evaluateFormula} from './swarm-expressions.js?v=0.8.0';
import {basicShape,samplePath,wordStrokes,validateStrokes,pathMetrics,largeFleetShape} from './swarm-shapes.js?v=0.8.0';

import {sequenceWords,wordSequenceState} from './word-sequence.js?v=0.8.0';
import {validateArt,artSlot} from './art-formation.js?v=0.8.0';
import {validateMusic} from './step-sequencer.js?v=0.8.0';
import {EFFECT_DEFAULTS,EFFECT_RANGES,MOTION_PATTERNS,SHOW_STYLES,clearProgramEffects,seededVariation} from './fleet-effects.js?v=0.8.0';

export const PROGRAM_SHAPES={ring:'Ring',wedge:'Wedge',line:'Line',grid:'Grid',column:'Column / trail','double-orbit':'Dual orbit',scatter:'Adaptive scatter',staggered:'Staggered','high-low':'High / low',overwatch:'Overwatch',word:'Word',drawing:'Drawing',pixels:'Pixel / image art'};
...
export function validateSwarmProgram(raw,{reset=false,aircraft=STARTER_AIRCRAFT,groups={}}={}){
 ...
 if(!raw||raw.version!==1||!['manual','script'].includes(raw.mode)||!Array.isArray(raw.ids)||(raw.ids.length<1&&aircraft.length>0)||raw.ids.length>aircraft.length||new Set(raw.ids).size!==raw.ids.length||raw.ids.some(id=>!knownIds.has(id)))throw Error('Choose valid aircraft from this fleet.');
 ...
 const strokes=validateStrokes(raw.strokes),art=validateArt(raw.art),music=validateMusic(raw.music);
 if(settings.shape==='pixels'&&!art?.points.length)throw Error('Open Art and paint or upload an image first.');
 ...
 return {...base,mode:raw.mode,ids:[...raw.ids],settings,strokes,art,music,source:raw.source,...(aircraft!==STARTER_AIRCRAFT?{fleetIds:[...aircraft],groups:Object.fromEntries(Object.entries(groups).map(([key,ids])=>[key,[...ids]]))}:{}),...(!reset?{enabled:raw.enabled,running:raw.running,time:raw.time,activeIds:[...new Set(raw.activeIds)]}:{})};
}
...
export function compileProgram(source,{aircraft=STARTER_AIRCRAFT,groups={}}={}){
 ...
 for(let row=0;row<lines.length;row++){
  const text=lines[row].split('#')[0].trim();if(!text)continue;
  ...
  try{
   ...
   if(command==='select'){const value=args[0];group=value==='all'?[...aircraft]:Object.hasOwn(groups,value)?[...groups[value]]:value==='scouts'?aircraft.filter(v=>v.startsWith('scout')):value==='relays'?aircraft.filter(v=>v.startsWith('relay')):value.split(',');if((!group.length&&aircraft.length>0&&!Object.hasOwn(groups,value))||group.some(id=>!aircraft.includes(id))||new Set(group).size!==group.length)throw Error('Unknown aircraft group.');continue;}
   ...
   if(command==='formation')patch.shape=choice(args[0],ENUMS.shape,'formation');
    ...
    const slot=command==='layer'?number(args.shift(),1,4,'layer'):1;if(!Number.isInteger(slot))throw Error('Layer must be a whole number, 1–4.');
    if(args.length!==3&&!(args[0]==='none'&&args.length===1))throw Error('Use influence none, layer 2 none, or layer 2 wave 8 0.6.');
...
export function programOptions(program,id,time=program.time){
 ...
 if(program.mode==='script'){
  const script=compiledFor(program);opts.countIn=script.countIn??opts.countIn;
  const absolute=Math.max(0,time-opts.countIn),t=script.repeat?mod(absolute,script.repeat):absolute;
  // Preserve the outgoing performance for a physical, gradual formation handoff.
  if(script.repeat&&absolute>=script.repeat&&DIRECTOR_STYLES[opts.show])for(const cue of script.cues)if(cue.patch.show&&cue.idSet.has(id))opts.show=cue.patch.show;
  for(const cue of script.cues)if(cue.at<=t&&cue.idSet.has(id)){
   if(cue.patch.show&&cue.patch.show!==opts.show){opts.previousShow=opts.show;opts.showElapsed=t-cue.at;}
...
export function influenceVector(opts,point,time,index,count){
 const [x,y,z]=point,t=time,q=t*opts.frequency+opts.phase+index*.7,a=opts.strength*opts.blend,r=Math.max(1,Math.hypot(x,z));let v=[0,0,0];
 ...
 if(opts.field==='lissajous')v=[Math.sin(q*2),Math.sin(q*3)*.4,Math.cos(q*3)];
 if(opts.field==='spiral')v=[Math.cos(q),Math.sin(q*.5)*.35,Math.sin(q)];
 if(opts.field==='braid'){const side=index%2?1:-1;v=[Math.sin(q)*side*(.4+.6*(1+Math.sin(t*.3))/2),Math.cos(q)*.35,Math.sin(q*.5)*.5];}
 if(opts.field==='twin'){const a1=[Math.cos(t*.4)*25,Math.sin(t*.4)*25],a2=[-a1[0],-a1[1]],weight=(1+Math.sin(q))/2;v=[(a1[0]*weight+a2[0]*(1-weight)-x)/40,.25*Math.sin(q),(a1[1]*weight+a2[1]*(1-weight)-z)/40];}
 if(opts.field==='square'||opts.field==='riemann'){const u=x/opts.fieldScale,w=z/opts.fieldScale,d=1+u*u+w*w;v=opts.field==='square'?[u*u-w*w,0,2*u*w]:[2*u/d,(u*u+w*w-1)/d,2*w/d];}
 if(opts.field==='custom'){
  const vars={x,y,z,t,i:index,n:count,phase:opts.phase};
  v=['formulaX','formulaY','formulaZ'].map(key=>evaluateFormula(formula(opts[key]),vars)*opts.blend);
...
export function sampleSwarmProgram(program,id,{time=program.time,reducedMotion=false}={}){
 ...
 const variationIndex=programIndex(program.fleetIds||ids,id),variation=seededVariation(variationIndex,opts.seed),t=reducedMotion?0:Math.max(0,time-opts.countIn),motionTime=t*(1+variation*opts.speedVariance),phase=motionTime*opts.patternSpeed+opts.phase+variation*opts.phaseVariance,spacing=opts.spacing;
 ...
 if(opts.shape==='word'||opts.shape==='drawing'){
  ...
  const start=large?largeFleetShape('grid',i,n,spacing,opts.scale):basicShape('ring',i,n,spacing),blend=reducedMotion?1:opts.morph?clamp(t/opts.morph,0,1):1;
  local=shaped.map((value,j)=>start[j]+(value-start[j])*blend);
 }
 const pixel=opts.shape==='pixels'?artSlot(program.art,i,n):null;
 if(pixel){local=[...pixel.position];local[1]+=Math.max(0,(program.art.rows-1)*2-opts.height+10);}
 const unmoved=[...local];
 if(!reducedMotion){
 ...
 const angle=opts.rotation*Math.PI/180,[x,y,z]=local;
 local=[(x*Math.cos(angle)-z*Math.sin(angle))*opts.scale,y*opts.scale,(x*Math.sin(angle)+z*Math.cos(angle))*opts.scale];
 let field=[0,0,0],error='';try{
  for(let slot=1;slot<=4;slot++){const suffix=slot===1?'':slot;if(opts['field'+suffix]==='none')continue;
   const layer=slot===1?opts:{...opts,...Object.fromEntries(['field','strength','frequency','phase','blend'].map(key=>[key,opts[key+slot]]))};
   const vector=influenceVector(layer,local,reducedMotion?0:motionTime,i,n);field=field.map((v,j)=>v+vector[j]);
  }
  field=field.map((v,j)=>v*opts[['axisX','axisY','axisZ'][j]]);const length=Math.hypot(...field);if(length>32)field=field.map(v=>v*32/length);
 ...
 if(opts.show!=='none'&&!reducedMotion&&time>=opts.countIn){
  const beat=Math.max(0,motionTime-i*opts.offset)*(opts.beatSync?opts.bpm/60:1)*opts.showSpeed+variation*opts.phaseVariance,cycle=mod(beat,opts.beatSync?16:12),spin=clamp((cycle-4)/2,0,1)*Math.PI*2;
  if(opts.show==='flyby'){combined[0]+=Math.sin(beat*Math.PI/8)*32;combined[2]+=Math.cos(beat*Math.PI/8)*10;attitude.roll=spin;}
  if(opts.show==='corkscrew'){combined[0]+=Math.sin(beat*.8)*8;combined[1]+=Math.cos(beat*.8)*6;attitude.roll=Math.sin(beat*.8)*.65;}
  if(opts.show==='ribbon'){combined[0]+=Math.sin(beat*.4+i*.06)*18;combined[1]+=Math.cos(beat*.8+i*.04)*5;attitude.roll=Math.sin(beat*.4)*.5;}
  if(opts.show==='salute'){combined[1]+=(1-Math.cos(beat*.6))*6;attitude.pitch=Math.sin(beat*.6)*.3;}
 ...
 const directed=showPosition(opts.show,i,n,reducedMotion?0:t*opts.showSpeed);if(directed){const previous=showPosition(opts.previousShow,i,n,reducedMotion?0:t*opts.showSpeed),u=clamp((opts.showElapsed??opts.morph)/Math.max(.001,opts.morph),0,1),blend=u*u*(3-2*u);combined=previous?directed.map((v,j)=>previous[j]+(v-previous[j])*blend):directed;}
 combined=combined.map((v,j)=>beforeShow[j]+(v-beforeShow[j])*opts.showAmount+seededVariation(variationIndex,opts.seed,j+1)*opts.variance);attitude.pitch*=opts.showAmount;attitude.roll*=opts.showAmount;
 ...
 return {target:combined,base,field,attitude,opts,error,strokes,beaconHex:pixel?.color||null};
}
