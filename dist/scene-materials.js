...
export function surfaceMaterial(color, kind = 'concrete', extra = {}) {
  const m = new T.MeshStandardMaterial({ color, roughness: 0.85, metalness: 0, ...extra });
  const uniforms = { surfaceTime: { value: 0 }, nightLight: { value: 1 }, wetness: { value: 0 } };
  m.userData.surface = uniforms;
  ...
  m.onBeforeCompile = (shader) => {
    ...
    shader.fragmentShader =
      `varying vec3 surfacePosition;uniform float surfaceTime;uniform float nightLight;uniform float wetness;\n${SURFACE_NOISE}\n` +
      shader.fragmentShader;
    ...
    if (kind === 'grass')
      colorCode += `float stripe=step(.5,fract(surfacePosition.z/22.));diffuseColor.rgb*=mix(.86,1.08,stripe);diffuseColor.rgb=mix(diffuseColor.rgb,diffuseColor.rgb*vec3(.75,.8,.42),grNoise(surfacePosition*.12)*.3);`;
    if (kind === 'concrete')
      colorCode += `vec2 seam=abs(fract(surfacePosition.xz/8.+.5)-.5);float joint=1.-smoothstep(.005,.018,min(seam.x,seam.y));diffuseColor.rgb*=1.-joint*.22;`;
    if (kind === 'glass')
      colorCode = `vec2 cells=vec2((surfacePosition.x+surfacePosition.z)/3.4,surfacePosition.y/3.6);vec2 cell=fract(cells);float pane=step(.12,cell.x)*step(.14,cell.y)*step(cell.x,.86)*step(cell.y,.82);float occupied=step(.46,grHash(vec3(floor(cells),7.)));diffuseColor.rgb=mix(diffuseColor.rgb,vec3(.12,.2,.25),pane*.75);`;
    if (kind === 'water')
      colorCode = `float grain=grNoise(surfacePosition*.06);diffuseColor.rgb*=.8+.25*grain;`;
    ...
    if (kind === 'grass')
      bump = `float heightDetail=grNoise(surfacePosition*12.)*.055+grNoise(surfacePosition*1.8)*.08;`;
    if (kind === 'water')
      bump = `float heightDetail=sin(surfacePosition.x*.21+surfacePosition.z*.15+surfaceTime*.85)*.20+sin(surfacePosition.x*.48-surfacePosition.z*.3+surfaceTime*1.2)*.06+grNoise(surfacePosition*1.2+vec3(surfaceTime*.15,0,0))*.03;`;
    if (kind !== 'glass')
      shader.fragmentShader = shader.fragmentShader.replace(
